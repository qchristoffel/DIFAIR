...
from losses.new_loss_v0 import loss_belong


class NewPropositionHelper0(BaseLossHelper):
    """
    Using polynomial functions to compute the loss.
    """
    ...
    def predicted_class(self, pred, class_anchors=None):
        ...
        x_count = tf.reduce_sum(tf.where(x > 0.5 * self.anchor_val, 1.0, 0.0), axis=2)
        x_magnitude = tf.norm(x, axis=2)

        return tf.math.argmax(x_magnitude * x_count, axis=1)

    def osr_score(self, pred, class_anchors=None):
        x = tf.reshape(pred, (-1, self.nb_classes, self.nb_features))
        x_count = tf.reduce_sum(tf.where(x > 0.5 * self.anchor_val, 1.0, 0.0), axis=2)
        x_magnitude = tf.norm(x, axis=2)
        return tf.reduce_max(x_magnitude * x_count, axis=1)
